...
void measure_dis::startwork()
{
    ...
    // static int count = 0;
    int iCount = 1;
    int capture_left_tmp = 0;
    int capture_right_tmp = 0;
    int capture_left_tmp2 = 0;
    int capture_right_tmp2 = 0;
    ...
    */
    ...
    while(1) // 循环测距
    {
        ...
        capture_right_tmp2 = capture_right;
        mutex.unlock();
        if (capture_left_tmp && capture_right_tmp && capture_left_tmp2 && capture_right_tmp2 && !readyFlag)
        {
            readyFlag = true;
            digitalWrite(LEDPin, LOW);
            sleep(1);
